...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#id_pembelian').on('change', function(e) {
            ...
            console.log('change --find penjualan' + id)
            if (id == '') {
                alert('Pilih Penjualan Terlebih Dahulu')
            } else {
                $.ajax({
                    type: 'GET',
                    url: '<?= base_url('retur/penjualan/find/') ?>' + id,
                    dataType: 'JSON',
                        ...
                        for (let i = 0; i < data.length; i++) {
                            html += `<tr id ="rec-` + id++ + `"> 
                                ...
                                <td>
                                    ...
                                    <input type="hidden" value="` + data[i].id_penjualan + `" name="id_pnj[]"  class="form-control" readonly required>
                                    <input type="hidden" value="` + data[i].id_warna + `" name="id_warna[]"  class="form-control" readonly required>
                                    <input type="hidden" value="` + data[i].id_warna + `" name="id_warna[]"  class="form-control" readonly required>
                                    <input type="hidden" value="` + data[i].ready + `" name="ready[]"  class="form-control" readonly required>
                                ...
                                <td>
                                    <input type="text" name="harga_jual[]" value="` + format_number(data[i].harga_jual) + `" id="unit_price-" class="form-control form-calc unit_price" readonly required up-ke='0'>
                                </td>
                                <td>
                                    <input type="text" name="qty_penjualan[]" value="` + data[i].qty + `" id="unit_price-" class="form-control form-calc unit_price" readonly required up-ke='0'>
                                </td>
                                <td>
                                    <input type="number" name="qty[]" class="form-control form-calc qty" min="1" max="` + data[i].qty + `" id="qty-" value="1" required>
                                </td>
